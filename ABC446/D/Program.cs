...
class Program
{
    static void Main()
    {
        ...
        int N = sc.Int();
        long[] A = sc.LongArr(N);
        Dictionary<long , long> kazu = new();
        long max = 0;
        for(int i = 0;i < N;i++)
        {
            if(kazu.ContainsKey(A[i]-1))
            {
                if(kazu.ContainsKey(A[i])) kazu[A[i]] = Math.Max(kazu[A[i]-1] + 1, kazu[A[i]]);
                else kazu[A[i]] = kazu[A[i] - 1] + 1;
                kazu.Remove(A[i]-1);
                max = Math.Max(max,kazu[A[i]]);
            }
            else
            {
                if(!kazu.ContainsKey(A[i]))  kazu[A[i]] = 1;
                max = Math.Max(max,kazu[A[i]]);
            }
        }
