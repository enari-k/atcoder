...
class Program
{
    static void Main()
    {
        ...
        for (int j = 1; j <= N + 1; j++)
        {
            for (int i = 1; i <= N + 1; i++)
            {
                masu[i, j] = masu[i, j-1] + masu[i, j];
            }
        }
        long answer = 25000000;
        long masume = N*N - masu[N+1,N+1];
        for(int k = 0;k <= N;k++)
        {
            long score = masu[k+1,k+1]*2 + masume - k*k; 
