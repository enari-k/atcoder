...
class Program
{
    static void Main()
    {
        ...
        for(int i = 0;i < Q;i++)
        {
            int A = sc.Int();
            int B = sc.Int();
            if(monster[A-1]>monster[B-1])
            {
                Console.WriteLine(LowerBound(iti,monster[A-1]) + LowerBound(iti,monster[B-1]));
            }
            else
            {
                Console.WriteLine(LowerBound(iti,monster[B-1])-LowerBound(iti,monster[A-1]));
            }
        }
        Console.Out.Flush();
    }

    static int LowerBound(List<double> list, double value)
    {
