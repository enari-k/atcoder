...
class Program
{
    static void Main()
    {
        ...
        List<int> miho = new();
        int point = 0;
        List<char> mozi1 = new(),mozi2 = new();
        foreach(char mozi in S)
        {
            if(mozi == 'A')
            {
                point++;
        ...
        long answer = 0;
        bool flag = true;
        if(mozi1.Count == mozi2.Count)
        {
            for(int i = 0;i < mozi1.Count;i++)
            {
                if(mozi1[i]!=mozi2[i]) flag = false;
            }
        }
        else
        {
